...
const cakes = (recipe, avail) => {
  const counter = [];
  for (ingr in recipe) {                                      //  LOOP THROUGH KEYS IN recipe
      if (!avail[ingr]) return 0;                             //  if NO SUCH ingr IN AVAILABLE return 0
      counter.push(Math.floor(avail[ingr] / recipe[ingr]));   //  OTHERWISE PUSH QUANTITY IN ARR
  }
  return Math.min(...counter);                                // return SMALLEST QUANTITY AS RESULT
};
